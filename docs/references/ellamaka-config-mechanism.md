...
> 模型发现不再兼容 `OPENCODE_MODELS_*` 或 `MODELS_DEV_API_JSON`。运行时只接受有效、非空的 provider catalog；默认缓存可自行恢复，而显式路径与仓库 `.ci/models.json` snapshot 始终是只读输入。构建不会自动刷新或回写该 snapshot。
