...
### ELL-G12: Desktop sidecar 启动失败时 onboarding 无兜底入口（P2）

**Current**: onboarding HTTP 面与 Workbench 同由 sidecar 承载，sidecar 随应用无条件启动。sidecar 自举失败时主窗口没有本地诊断兜底页，onboarding 无法经网络探测展开，新机器上呈现为不可诊断的启动失败。

**Target**: sidecar 启动失败时 Desktop 展示本地诊断页（错误摘要 + 重试入口），不静默白屏；sidecar 恢复后诊断页引导进入 onboarding 或 Workbench。

**Design**: `./DESIGN-onboarding.md`（Desktop 接入节）

**Exit**:
- [ ] sidecar 启动失败时主窗口出现诊断页而非空白
- [ ] 诊断页提供重试路径，恢复后可进入正常流程
