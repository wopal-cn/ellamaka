...
> **研究结论**：本报告只承载研究结论（解构、审计、复刻、选型、语义分析），设计决策见 dsh 设计文档集（`../DESIGN.md`、`../DESIGN-dsh-base.md`、`../DESIGN-dsh-web.md`、`../DESIGN-ellamaka-tools.md`）。
...
## DeepSeek Harness (`dsh`) 底座与微内核全景解构
...
> **勘误（2026-08-16）**：本节表格原文标注为"实测"，但写作当时并无实测证据——dsh 仓库 CI（`.github/workflows/`）全部为 Node（node24），无任何 Bun 配置，下表实为基于依赖类型的推断性结论。后续已由真实冒烟测试补验部分内容：`@deepseek-ai/cordis@4.0.1` 在 Bun 1.3 下服务注册、inject 依赖、事件系统、`ctx.fiber.dispose()` 生命周期全链路可用（init 1.79ms）。插件级兼容性未逐包实测，实际挂载时须按契约符合性冒烟测试逐个验证（见 [工具容器设计](../DESIGN-ellamaka-tools.md)）。
...
> 早期融合方案（换心手术 + 4 大网桥 + dsh 插件生态挂载）经深度审计后修正。以下为审计修正的研究结论，设计决策见 dsh 设计文档集（`../DESIGN.md`、`../DESIGN-dsh-base.md`、`../DESIGN-dsh-web.md`、`../DESIGN-ellamaka-tools.md`）。
...
| **task/subagent** | task.ts 内部 session 复用，单后端 | 六后端 Provider 架构（[闪光点清单（源码实证）](#闪光点清单源码实证)） | 多后端架构优，但见 [逐项复刻判定](#逐项复刻判定)（B 类复刻路径） |
...
换来的独占收益（按价值排序）：① 确定性对拍回归体系；② dsh 深耦合插件零适配直挂（[深耦合包机制复刻研究（2026-08-16 补充）](#深耦合包机制复刻研究2026-08-16-补充) 六项复刻约 4-6 周工作量全免）；③ 崩溃续跑 + 时间旅行调试；④ KV-cache 友好的 epoch 请求组装。诚实折扣：收益②本就是复刻路线预算内的活；收益①③是真正独占，但可被中间路线部分获得（[会计类比与薄账本中间路线](#会计类比与薄账本中间路线)）。
...
## cordis 内建日志系统与插件日志集成机制（2026-08-17 补充）
...
## dsh Web 前端插件架构剖析（2026-08-20，源码级）
...
### 层次：配置面 → host 面 → agent 面 → dsh.client 面
...
### 是否必须同一容器？node half 必须，browser half 天然跨容器
...
### 动态配置与加载：声明式 patch + 按需拉取 + 增量重扫 + HMR
...
### 动态能力是否会因固化前端 bundle 而丧失？取决于固化哪一层
...
## 权威参考资料与考证证据链清单 (Canonical References & Evidence)

为了方便后续专家与多 Agent 评审团进行严密的代码交叉审查与事实考证，特此整理本研究所依赖的全部源码、架构规范与设计笔记索引：

### DeepSeek Harness (`dsh`) 官方源码与子系统规范
* **微内核与架构总览**：
...
## 容器集成机制深度研究（2026-08-20，源码级）

> **研究定位**：回答"ellamaka 如何消费 cordis 容器内插件能力"的机制问题。涵盖 dsh 工具注册表 API、ellamaka 工具管道实证、三层插件实例化模型、同进程通信机制、wopal-plugin 形态核查、方案演进中被推翻的假设、待验证猜想清单。本节内容为源码实证或明确标注的待验证猜想；集成方案的形态决策见 dsh 设计文档集；执行计划见空间 Plan 索引 `PLAN-TODOS.md`。

### dsh 工具注册表：完整的动态消费 API（源码实证）
...
| 容器=进程级单例，wopal 插件须单例+分片改造 | 三层实例化模型（[三层插件实例化模型（★ 推翻"容器=进程级单例"假设）](#174-三层插件实例化模型★推翻容器进程级单例假设)） | per-directory scope 挂载，与原生形态同构；standing 共享为优化开关 |
...
### 容器插件对 session 的依赖图谱（2026-08-21，源码级审计）
...
- 依赖 dsh 自己的 loop/session 运行时（事件日志语料重放、agent.send 唤醒通道、子会话模型），契约桥翻译不了引擎层语义（[审计修正的关键发现](#审计修正的关键发现) C2）。**唯一**能启用它们的方式是让 dsh 引擎真实驱动轮次（ellamaka 退化为 shell）。
...
| **B2（引擎接管）** | 模式一 + 二 + 三 | 高（ellamaka 退化为 shell） | 唯一启用引擎层插件的方式；属 [`DESIGN.md` 加载契约](../DESIGN.md#ontology-loading-contract) 载体决定（推迟），且 dsh 界面已集成 workbench（双核心可选），B2 的"界面"价值已由 iframe 承接 |
...
#### dsh 工具消费的 `exec.agent` 契约面（2026-08-21，源码实证）

> **用途**：B1 session 门面设计的契约对照证据（设计定稿见 [工具容器设计](../DESIGN-ellamaka-tools.md)）。本节只记录 dsh 工具实际消费的 `exec.agent` 面与 ellamaka 侧可提供的数据，不含设计决策。
