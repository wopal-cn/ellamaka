...
> **用途**：ellamaka × dsh 双引擎融合进度索引与批次管理。
> **阶段状态**：PoC 探索与融合验证阶段已正式结束，核心架构与基础设施已全量合并入主线（`main`）。后续开发按正式 dev-flow Plan 与各主线（A/W/E/S）持续演进。
> **分工**：`DESIGN-ellamaka-dsh.md` 管设计真相（按标题引用，不使用章节号）；dev-flow Plan（`.wopal-space/plans/ellamaka/`）管跨文件、多任务的大步实施；本文件管总览与执行顺序。
...
设计真相见 `DESIGN-ellamaka-dsh.md`「唯一 home 与目录所有权」「插件供应链」「dshmarket 插件市场接入」。**决策**：dsh 插件生态全面对齐官方契约（声明与终态按官方语义），执行器换成 Bun（零 pnpm、零官方 dsh CLI）；home 布局对齐（`DSH_HOME=$WOPAL_HOME/dsh/home`）。对齐原则：凡是官方生态有既定机制的，对齐它，不自研平行路。
...
设计真相见 `DESIGN-ellamaka-dsh.md`「wopal 插件包（Agent 配置随包发布）」。主战场是 dsh 界面本身。
...
设计真相见 `DESIGN-ellamaka-dsh.md`「多空间解耦与实验 profile」。「助理」tab 的遮蔽耦合是 P7 的设计债；实验性第三方插件（依赖历史闭包、安全边界存疑）不应当进主 Web 容器。E 线拆掉遮蔽、把 DSH 空间配置化，并以独立进程承载实验 profile。
...
**执行顺序**：E1 → E2。E1 不依赖 A 线施工内容（A 线动 home/安装器，不碰 tab 模型），可与 A 线并行排期；E2 依赖 E1 的前端 tab 模型扩展、A2 的官方声明供应链，以及「物化指定历史版本闭包」的新能力。W4 是 E2 的首个实证消费者（通过实验空间评估外部插件）。E 线立项（2026-09-06），设计已落入 `DESIGN-ellamaka-dsh.md`。

---

## 门槛轨道：G 线（workbench × dsh 前端插件互通）

设计真相与执行清单见 `DESIGN-ellamaka-dsh.md`「workbench × dsh 前端插件互通」（V1–V5）。
...
设计真相见 `DESIGN-ellamaka-dsh.md`「浏览器认证（rc.1 browser-auth）· 认证联盟的架构定位」。Basic（ellamaka 外层）与 browser-auth cookie（dsh 内层）双信任域格局定稿，不做 Basic-only 统一；修补联盟的四个缺口。auth-fix-3 是 E 线（实验 profile 多挂载）的前置。
...
设计真相见 `DESIGN-ellamaka-dsh.md`「壳单端口化与 workbench 精简」。终局形态「一个 runtime，N 个壳」：desktop 删除 4123 代理层与 `oc://` 渲染宿主，renderer 加载 sidecar 端口上的 workbench；官方 opencode app 移除，`/` 变设备协商前门；i18n/theme 数据收敛。Phase 1 保留 sidecar 构建链（dsh 生态 Bun 兼容未收敛，desktop 引擎需 node 环境），Phase 2（收敛后）引擎产物唯一化、sidecar 构建链退役——本线只排 Phase 1 与精简项，Phase 2 待 Bun 兼容收敛后另立批次。
...
| **S4** | renderer 迁移 + 4123 退役 | packaged renderer 改 loadURL `http://127.0.0.1:<sidecarPort>/workbench`（onboarding 仍 oc://，转场顺序导航）；删除 `dshHttpProxy`/`createDshProxy`/cookie jar/`platform.dshProxyOrigin` 全链路/`dsh-surface.tsx` 的 `oc:` 分支；Electron 加固复核（will-navigate 守卫、preload 暴露面） | 打包 desktop 无 4123 监听；DSH iframe 同源（cookie/WS 浏览器原生处理）；两个 desktop 实例并行运行互不冲突；localStorage 重置一次性接受（经方案裁定） | dev-flow Plan（依赖 S3；TDD + rook 审查 + 实机验收） | 待排期 |
...
| 事项 | 说明 | 关联设计 |
|------|------|----------|
| per-角色武器装备（自定义招人 provider） | 仅当 W1–W4 落地后仍存在「队员必须带不同武器」的真需求时才立项 | DESIGN-ellamaka-dsh「组队语义」 |
| 界面演进（iframe → 原生） | W3 的插槽纪律保证定制件可无损迁移 | DESIGN-ellamaka-dsh「空间皮肤插件」 |
| 插件 entry 级配置命令（`plugin config`） | 供应链 Plan 的 Out of Scope，独立后续 | DESIGN-ellamaka-dsh「插件供应链 · 配置与信任」 |
| dsh 协处理器（借引擎跑子任务） | 不紧急；涉及托管会话语义设计，暂挂起 | — |
| pnpm-lock 兼容生成 | 若市场更新自动回滚的降级语义不可接受，评估 Bun 安装器产出最小合法 pnpm-lock | DESIGN-ellamaka-dsh「插件供应链 · 已知偏差」 |
...
| 2026-09-04 | B1（拆雷）+ B1.5（dump-config）完成。事故教训：运行中引擎 `profiles/` 是引擎领地——内容幂等写 ≠ mtime 幂等，standing 重建按 mtime/size 触发；测试/dump/诊断一律 temp-home 注入（Live-home quarantine 已写入 AGENTS.md） |
| 2026-09-05 | B3 完成（rc.1 闭包 + browser-auth）；B1.5 分支合并回主线 |
...
| 2026-09-06 | **E 线立项（多空间解耦与实验 profile）**：P7「助理」tab 遮蔽耦合确立为设计债；DSH 空间配置化（settings.jsonc 默认值 + 设置面板覆盖）；实验 profile 独立进程 + 独立 DSH_HOME + 服务器注册式空间 tab；设计落入 DESIGN-ellamaka-dsh.md，暂不排期（用户裁定 E 线独立立项，暂不写 plan） |
...
| 2026-09-06 | **S 线立项（壳单端口化与 workbench 精简）**：desktop 删 4123 硬编码代理端口与 `oc://` 渲染宿主，renderer 加载 sidecar 端口 workbench（Phase 1 保留 sidecar 构建链——dsh Bun 兼容未收敛，Phase 2 收敛后引擎产物唯一化）；官方 app 移除 + `/` 设备协商前门（移动 UA → /dsh/，桌面 UA → /workbench；/dsh 不升根——前缀自治不变量、适配层改写已完成、B5 信任域边界三重理由）；i18n 18→2 语言、theme 37→1 主题数据收敛；设计落入 DESIGN-ellamaka-dsh.md「壳单端口化与 workbench 精简」+ 设计约束 #23；批次 S1（精简数据层）→ S2（官方 app 移除 + 设备路由）→ S3（sidecar serve SPA）→ S4（renderer 迁移 + 4123 退役），S3 与 S1/S2 可并行 |
| 2026-09-07 | **A1/A2 状态校正**：两 plan 的 User Validation checkbox 均已勾选（用户验收通过），归档确认完整，PLAN-TODOS 状态列从"待用户验证"改为"已完成"。**223（dsh CLI 命令面对齐）归档**：用户验证通过，verify + archive `--keep-worktree` 完成。 |
| 2026-09-09 | **PoC 阶段正式收官与主线合并**：PoC 分支 `poc-ellamaka-cordis` 全量验证通过并合并入 `main`；双引擎融合架构、官方插件供应链与生产物化基线转为维护态；设计文档重构升级为 `DESIGN-ellamaka-dsh.md`，PoC 分支清理完毕，后续统一基于 `main` 分支推进正式 Plan 演进。 |
...
| 2026-09-07 | **A3 方案定稿（spawn 覆盖服务，取代暂缓诊断）**：深入研究 + spike 实证确认正解 = 宿主注入 `desktopPnpm`，其 `runPlugin` 内部 **spawn `ellamaka dsh plugin` 子进程**（市场 Desktop 路径零修改接管，官方扩展点）。关键实证：市场 `desktopPnpm.runPlugin` 契约是**流式 handle**（spawn 天然满足，无需改造 Bun 安装器）；Desktop 路径下 `probePnpm`/`provisionPnpm` 已被市场 stub 成 true/ok（不 spawn pnpm）；routes 层无绕过 runPlugin 的直接 pnpm spawn；spawn 实测预热后 0.28s（DB migration 一次性）；并发安全由 `plugins.lock` 跨进程 mutex 保证；进程隔离（子进程崩不影响引擎）。不用 wopal 薄封装（市场 spawn 不到 wopal，非宿主 argv）；不 fork 市场。A3 plan 技术上下文已更新为 spawn 方案。**新增前置：`ellamaka init` 命令（接受 `WOPAL_HOME`，新 home 物化 dsh 闭包 + 初始化）**——为隔离 home 端到端验证铺路（当前隔离 home 缺闭包 heal，spike 已证是 peer 校验失败根因）。 |
| 2026-09-07 | **`ellamaka dsh init` 命令落地（A3 物化前置）**：命名最终定为 dsh 组子命令（非顶层 `ellamaka init`——init 物化/初始化的是 dsh 闭包与 home 布局，归 `ellamaka dsh` 家族语义内聚，与 plugin/dump-config 同级；user 裁定）。命令面：`ellamaka dsh init [--home <dir>]`，默认 home = `Global.Path.wopalHome`（即 `$WOPAL_HOME`/`~/.wopal`），`--home` 显式覆盖便于隔离验证；engine-free（`light`，不 boot AppRuntime），仅调 `initializeDshRuntime`（manager 统一物化入口，entry=`init`）物化闭包 + seed profile 模板后退出，无 mount。变更：`dsh-init.ts`（新命令 + 可注入 `runDshInit`）、`index.ts` dsh group 注册、manager `InitializeDshOptions.entry` 扩 `"init"`、`dsh-init.test.ts`（定义契约 + `ELLAMAKA_DSH=0` 零文件 gate）。typecheck 零错、44 现有 dsh 测试无回归；**真实物化 smoke**：temp home 下载 582 packages 物化成功，`dsh.init.ready entry=init` + profile `web,ellamaka-tools` seed。隔离 home 端到端验证（spawn `ellamaka dsh plugin add` 装 better-sidebar）留待 A3 执行。 |
| 2026-09-07 | **实证推翻「缺闭包 heal 导致 peer 失败」归因 + resolver 双缺陷修复**：隔离 home `dsh init` + `add dsh-better-sidebar` 实测，peer 失败真因不是共享层缺失（init 已 seed profile 模板 + 物化闭包；resolver 根本不经 registry 以外路径），而是 **ellamaka 自己的 resolver 两个缺陷**：(1) 把官方 `@deepseek-ai/*` peer 当普通 registry 依赖解析——user 插件（better-sidebar）peer 全是官方平台组件（13 个，闭包已含 0.1.2-rc.1），DESIGN:614/673/D-05 明定官方 peer 由共享层 heal 满足不下载，resolver 违反既定策略（installer 装树时 `isOfficialPackage` 已跳过，resolver 解析树却没跳）→ 补 heal 不能解此错；(2) 手写 semver range 只支持 exact/caret/tilde/x，缺 OR（`\|`）、hyphen/`>=`、带 prerelease 的 caret——better-sidebar→mermaid 树用 `^11 \|\| ^12 \|\| ^13 \|\| ^14`，自研对 `^0.1.2-rc.1` 判 false（npm 真值 true），必然 NoVersionError。**修复（提交 `5d6092b2e9`）**：(1) resolver `enqueueDeps` 跳过 `@deepseek-ai/*`（对齐 installer）；(2) range 匹配换 **node-semver**（semver 已是 monorepo 依赖，非新依赖；ellamaka-cordis 本就用 yaml/chokidar/pacote；`--compile` 二进制 spike 验证可用，无 Arborist 忙循环；包管理 tag 语义 latest/dist-tags/`*` 留在本文件，因 node-semver 无 tag 概念）；顺带修正手写 `^0.0.x` caret 上界 bug（`^0.0.3` 应= `<0.0.4` 只匹配 0.0.3，原实现误收 0.0.4+）。**端到端**：隔离 home init + add 成功落盘 better-sidebar@0.18.0 官方终态（profile deps+bundles+实体）。typecheck 零错、68 plugins 测试无回归。**教训**：spike/A3 plan 归因「隔离 home 缺闭包 heal」不准——真实 root 是 resolver 把官方 peer 拉 registry + 手写 semver 缺语法；A3 plan 技术上下文该处表述待后续修正。 |
| 2026-09-07 | **resolver 二次缺陷修复：peer 依赖彻底不下载（提交 `a4f3cbf4ed`）**：上一条修复只跳过官方 `@deepseek-ai/*` peer，但 DESIGN:673 原意是「peer 依赖不下载、不摆放，交给运行时 parent-walk（闭包共享层）」——**非官方 peer 也不该被安装器下载**。实证：`add dsh-codex-connect@0.1.0-alpha.4.28` 卡死（40s+ 无输出）——其 peer `@earendil-works/pi-ai` 拉拽 openai/anthropic/aws-sdk-bedrock/google-genai 全生态 191 包进树，闭包本已含 react@18.3.1 / pi-ai@0.84.4 / schemastery@3.18.2 全部命中 peer 范围。**修复**：resolver `enqueueDeps` 只遍历 `dependencies` + `optionalDependencies`，彻底移除 `peerDependencies`（官方与非官方一律不下载）。**效果**：dsh-codex-connect 解析从 191 包/40s → **2 包（root+undici）/1.2s**，端到端 install 4.4s，官方终态正确（deps+bundles+实体，undici 落插件自身 node_modules）。resolver 15 测试全绿（改重写 peer 测试拥抱全跳过语义），installer 全 68 测试无回归。 |
