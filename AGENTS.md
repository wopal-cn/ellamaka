...
- **Trace is opt-in and categories are bounded**: `TRACE` is the fifth level, below `DEBUG`. It carries the high-volume lifecycle records (event-bus publishes, permission decisions) that normal operation must not emit. Emit them via `log.trace(category, message, extra)`; a record is written only when the effective level is `TRACE` and its category is selected (`--trace <categories>`, `all`/`*`, or an explicit `--log-level TRACE` with no selector). Use a stable, short category name shared across call sites
- **Trace never carries payloads or user data**: bus trace records the event type only; permission trace records the permission name, decision `action` (`allow|ask|deny`), an `escalated` marker, a count, and the reply outcome. Never write the event payload, evaluated pattern, command, path, session id, or pending request contents. An explicit `--log-level` always wins over `--trace` promotion
...
When diagnosing serve, TUI, or sidecar behavior, widen output through the log level rather than adding temporary records to the code.
...
| All trace categories | `ellamaka serve --log-level TRACE` | No selector means every category |
...
- If a category's records are missing, confirm the level actually resolved to `TRACE` (an explicit `--log-level` overrides `--trace`) and that the category is in the selector.
