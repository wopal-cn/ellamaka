...
export async function probeGithubAuthentication(homePath?: string, deps: GithubAuthenticationProbeDeps = {}) {
  const t0 = Date.now()
  ...
  step(
    `GitHub CLI 探测完成（${Date.now() - ghCliT}ms）: ${ghCli.authenticated ? "已认证" : ghCli.installed ? "已安装未认证" : "未安装"}`,
  )
...
export function detectGithubToken(
  ...
  try {
    const shellEnv = deps.loadUserShellEnv ? deps.loadUserShellEnv() : loadShellEnv(getUserShell())
...
export function readEnvConfig(envPath: string) {
  ...
  try {
    ...
    const hasAnyConfig =
      "WOPAL_MEMORY_ENABLED" in envVars || Boolean(llmEndpoint || llmModel || embeddingModel || hasLlmKey)
...
export function createOnboardingIpcHandlers(deps: OnboardingIpcDeps = {}) {
  ...
  const defaultExecuteStep: StepExecutor = async (step, input, onProgress, abortSignal) => {
    ...
    switch (step as string) {
      ...
      case "install-cli": {
        ...
        const ellamakaRes = normalizeSetupResult(
          await runSetupOperation({
            binaryPath: binPath,
            operation: "install-engine",
            input: payload,
      ...
      case "ontology-setup": {
        ...
        if (ontRes.status === "completed" || ontRes.status === "reused") {
          ...
          try {
            runtimeRes = normalizeSetupResult(
              await runSetupOperation({
                binaryPath: binPath,
  ...
  return {
    ...
    "onboarding-probe": async (_event: unknown, kind: string) => {
      ...
      process.env.WOPAL_HOME = homePath
      const isWin = process.platform === "win32"
      const _binPath = join(homePath, "bin", isWin ? "wopal.exe" : "wopal")
      ...
      switch (kind) {
        ...
        case "ontology": {
          ...
          trace(
            homePath,
            "ontology-probe",
            `空间能力本体检查完成（${Date.now() - inspectT}ms）${fromSnapshot ? "（快照命中）" : "（首次 inspect）"}`,
          )
          if (!inspection) {
            return {
              status: "broken",
        ...
        case "environment": {
          try {
            ...
            const availableTypes = hasAvailableTypes ? inspection.availableTypes : [{ type: "common", branch: "main" }]
            return {
              availableTypes,
      ...
      const run = async (): Promise<OnboardingStepResult> => {
        ...
        try {
          ...
          const res = await executor(
            stepName,
            input,
            (p) => deps.broadcastProgress?.({ step: stepName, ...p }),
