...
- **SSE event tiering**: high-frequency property changes (title, message stream) are handled locally by the corresponding component; structural events (`session.created` / `session.deleted` / `session.updated` with `timeArchived`) trigger SessionTree refresh. `message.part.*` only updates the corresponding PanelChat. SSE events must not trigger unrelated Panel or tree-level reloads.
