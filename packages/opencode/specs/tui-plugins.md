...
- Author package entrypoint is `@wopal/ellamaka-plugin/tui`.
...
- Import types from `@wopal/ellamaka-plugin/tui`.
- `@wopal/ellamaka-plugin` exports `./tui` and declares optional peer deps on `@opentui/core` and `@opentui/solid`.

Minimal module shape:

```tsx
/** @jsxImportSource @opentui/solid */
import type { TuiPlugin, TuiPluginModule } from "@wopal/ellamaka-plugin/tui"
...
When `plugin` entries exist in a writable `.opencode` dir or `OPENCODE_CONFIG_DIR`, OpenCode installs `@wopal/ellamaka-plugin` into that dir and writes:
...
That is what makes local config-scoped plugins able to import `@wopal/ellamaka-plugin/tui`.
...
- For generic config-to-bindings helpers, import `createBindingLookup` from `@wopal/ellamaka-plugin/tui`.
